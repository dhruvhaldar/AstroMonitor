        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    // Bolt Optimization: O(1) status check using counts
                    let (status_text, status_color) =
                        if self.alert_counts[level_index(AlertLevel::Critical)] > 0 {
                            ("SYSTEM CRITICAL 🔴", egui::Color32::RED)
                        } else if self.alert_counts[level_index(AlertLevel::Warning)] > 0 {
                            ("System Warning ⚠️", egui::Color32::YELLOW)
                        } else if self.alert_counts[level_index(AlertLevel::Info)] > 0 {
        ...
        match result {
            Ok(packet) => {
                ...
                if let Some(event) = monitor.check(&packet) {
                    ...
                    alert_counts[level_index(event.level)] += 1;
                }
            }
            Err(e) => {
                if let Some(idx) = index {
