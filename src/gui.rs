    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if !self.paused && self.packet_index < self.packets.len() {
                ...
                && steps < max_steps
            {
                ...
                // SAFETY: self.packets contains data generated by simulation.rs using string literals (valid UTF-8).
                let result = unsafe { Parser::parse_trusted(&self.packets[self.packet_index]) };
                Self::process_result(
                    &mut self.logs,
                    &mut self.alerts,
                                    ...
                                    match log {
                                        LogEntry::SimulatedPacket(idx) => {
                                            if let Some(packet_data) = self.packets.get(*idx) {
                                                ...
                                                if let Ok(packet) = unsafe { Parser::parse_trusted(packet_data) } {
