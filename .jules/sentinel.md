...
## 2025-02-14 - Log Flooding Denial of Service
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in `src/gui.rs` where incoming telemetry alerts and error messages were pushed to the `logs` VecDeque without checking `MAX_LOGS`, leading to unbounded memory exhaustion during alert floods.
**Learning:** Even if some standard logging paths (like simulated packets) handle queue bounds checking properly, all code paths (including alerts and parsing errors) that push elements to a global queue must independently enforce queue size constraints.
**Prevention:** Centralize queue size management or ensure all push operations (`push_back`) are immediately preceded by bounds checking/popping (`if queue.len() >= MAX_LIMIT { queue.pop_front() }`).
