...
## 2025-05-28 - CSV Injection Bypass via Quotation Marks
**Vulnerability:** The CSV injection sanitization logic checked if the payload started with a dangerous character after stripping standard whitespace and control characters. However, it failed to account for leading quotation marks (`"` and `'`). If an attacker prepends a quotation mark before a malicious formula (e.g., `"=cmd...`), spreadsheets will strip the quote and evaluate the inner formula, bypassing the filter.
**Learning:** Spreadsheets employ complex parsing rules that strip leading structural characters (like quotes) before evaluating cell content. Sanitization routines must mirror this parsing behavior by stripping quotes before checking for formula execution triggers.
**Prevention:** Extend the prefix validation skip-list in sanitization functions to explicitly skip over single (`'`) and double (`"`) quotation marks, in addition to whitespace and control characters, before assessing the first active character of the string.
